...
LTexture gShieldItemTexture;   // 道具圖
LTexture gShieldEffectTexture; // 特效圖
LTexture gHPIconTexture; // 血量圖示
...
//                    if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_SPACE) {
////                        std::vector<Bullet*> newShots = gpPlayer->fire(&gBulletTexture);
//                        int startX = gpPlayer->getPosX() + 30; // 假設主角寬60
//                        int startY = gpPlayer->getPosY();
//                        std::vector<Bullet*> newShots = PatternHelper::createPlayerShotgun(startX, startY, &gBulletTexture);
//                        gBullets.insert(gBullets.end(), newShots.begin(), newShots.end());
//                        Mix_PlayChannel( -1, gLaserSound, 0 );
//                    }
                    ...
                    {
                        std::vector<Bullet*> newShots; // 用來接子彈
                        ...
                        // S 鍵：Pattern S
                            ...
                            Mix_PlayChannel(-1, gLaserSound, 0); // 或者換一個音效
                        }
                        // D 鍵：Pattern D
        ...
        if (gCurrentState == STATE_PLAYING)
        {
            ...
            // 更新 Boss
            static float gBossSpinAngle = 0; // 螺旋彈的角度
            static int gBossFireTimer = 0;   // 發射計時器
            ...
            if (gpBoss) {
                ...
                // 情況 A：Boss 1 使用舊的寫法 (邏輯寫在 boss.cpp 裡，內建計時器)
                if (gLevelStage == BOSS_1_FIGHT) {
                    bShots = gpBoss->fire(&gEnemyBulletTexture);
                }
                // 情況 B：Boss 2 使用新的「螺旋彈」 (PatternHelper)
                    ...
                    // 設定發射頻率：每 10 幀發射一次 (數字越小射越快)
                    if (gBossFireTimer > 10) {
                        gBossFireTimer = 0; // 重置計時器

                        // 取得 Boss 中心點 (假設 Boss 圖片寬高約 100x100)
                        // 你可以用 gpBoss->getWidth() 取得更精準數值
                ...
                // 情況 C：Boss 3 使用新的「圓形擴散」或「散彈」
                else if (gLevelStage == BOSS_3_FIGHT) {
                    gBossFireTimer++;
                    // 設定為 90 幀 (約 1.5 秒) 發射一次，因為這個 Pattern 子彈很多
                    if (gBossFireTimer > 150) {
                        gBossFireTimer = 0;

                        int bx = gpBoss->getPosX() + 50; // 調整到 Boss 中心
                        int by = gpBoss->getPosY() + 80;

                        // --- 修改這裡：呼叫 Pattern C ---
                        bShots = PatternHelper::createBossPatternC(bx, by, &gEnemyBulletTexture); // 傳入 Boss 3 的圖片
                    }
                }
            ...
            for (size_t i = 0; i < gShields.size(); i++) {
                ...
                if (SDL_HasIntersection(&pRect, &sRect)) {
                    gpPlayer->addShield(1); // 增加 1 層防護罩
        ...
        else if (gCurrentState == STATE_TUTORIAL)
        {
           ...
           // 1. 畫 UI 文字 (移除 HP 顯示，只留 Score 和 Shield)
            ...
            // 2. 畫 HP 圖示 (放在左下角)
            int hp = gpPlayer->getHealth(); // 取得當前血量
            int iconX = 10;                 // 起始 X 座標 (左邊界)
            int iconY = SCREEN_HEIGHT - 40; // 起始 Y 座標 (視窗底部往上 40 pixel)
            int padding = 5;                // 圖示之間的間距
...
void close()
{
    ...
    gHPIconTexture.free();
    for(auto s : gShields) delete s; // 清空 vector
