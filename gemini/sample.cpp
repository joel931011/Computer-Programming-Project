...
static float gGMS_SpinAngle = 0.0f;   // 螺旋角度
static float gGMS_SpinSpeed = 0.01f;  // 旋轉速度
static bool  gGMS_SpinReverse = false;// 是否反轉 (同學程式有寫反轉邏輯)
static int   gGMS_Timer = 0;
        ...
        if (gCurrentState == STATE_PLAYING)
        {
            ...
            if (gpBoss) {
                ...
                if (gLevelStage == BOSS_1_FIGHT) {
                        // 1. 更新旋轉角度 (模擬同學的 base_angle += spin)
                        gGMS_SpinAngle += gGMS_SpinSpeed;

                        // 模擬同學的 spin 反轉邏輯 (可選)
                        if (gGMS_SpinAngle > 1.0f || gGMS_SpinAngle < -1.0f) {
                        // 這裡可以寫反轉邏輯，或者單純讓它一直轉
                    }

                    // 2. 發射計時器
                    gGMS_Timer++;
                    if (gGMS_Timer > 10) // 每 10 幀發射一次 (同學設 rate 5，這裡可自行調整)
                    {
                        gGMS_Timer = 0;

                        int bx = gpBoss->getPosX() + 69/2;
                        int by = gpBoss->getPosY() + 69/2;

                        // 3. 呼叫 PatternHelper
                        std::vector<Bullet*> newShots = PatternHelper::createGMSStarSpiral(bx, by, gGMS_SpinAngle, &gEnemyBulletTexture);
